...
export default class BetterCanvas extends Plugin {
				...
				menu.addItem((item) => {
					...
					item.onClick(() => {
						...
						newFileTask.then((newFile) => {
							...
							this.addChildNode(canvas, node, newFile);
						});
					});
				});
			}
	...
	addChildNode(canvas: Canvas, node: CanvasNode | null, newFile: TFile) {
		...
		console.log("node", node);
		if (node) {
			const newNode = createChildFileNode(canvas, node, newFile, "", 0);
	...
	replaceSelectionWithLink(newFile: TFile, editor: Editor) {
		const newFileLink = `[[${newFile.name}]]`;
